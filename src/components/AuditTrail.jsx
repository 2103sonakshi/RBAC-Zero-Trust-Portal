...
const AuditTrail = () => {
  ...
  const fetchAuditLogs = async () => {
    try {
      setLoading(true);
      // Fetch from blockchain instead of separate audit table
      ...
      if (response.data.success) {
        const logs = response.data.chain.map((block) => ({
          ...
          username: block.data.details?.username || block.data.userId,
          ip: block.data.ip,
    ...
    filteredLogs.forEach((log) => {
      ...
      if (hour < 5 || hour > 23) {
        suspicious.push({
          ...
          action: log.action,
          severity: "medium",
          message: `${log.action} at unusual hour (${hour}:00) from ${log.ip}`,
        });
      }
    });
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">
                  {new Set(filteredLogs.map((l) => l.ip)).size}
                </p>
              </div>
        ...
        <div className="mb-6 flex flex-wrap gap-3">
          ...
          <div className="relative group">
            <button
              disabled={exporting}
              className="px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-300 hover:bg-gray-700 transition-colors flex items-center space-x-2"
            >
              <Download className="h-4 w-4" />
              <span>Export</span>
            </button>
            <div className="absolute right-0 mt-2 w-48 bg-gray-800 rounded-lg shadow-lg border border-gray-700 hidden group-hover:block z-50">
              <button
                onClick={exportToPDF}
                disabled={exporting}
        ...
        {viewMode === "map" && (
          <div className="bg-gray-800 rounded-lg border border-gray-700 p-12 text-center">
            ...
            <div className="inline-flex items-center space-x-2 px-4 py-2 bg-gray-700 rounded-lg">
              ...
              <span className="text-sm text-gray-300">
                {new Set(filteredLogs.map((l) => l.ip)).size} unique IPs from{" "}
                {new Set(filteredLogs.map((l) => l.ip?.split(".")[0])).size}{" "}
                networks
              </span>
            </div>
          </div>
              ...
              <div className="relative bg-gray-800 rounded-lg max-w-2xl w-full p-6 border border-gray-700">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white bg-gray-750 p-3 rounded-lg border border-gray-700">
                  ...
                  <div>
                    ...
                    <pre className="text-xs text-gray-300 bg-gray-750 p-3 rounded-lg border border-gray-700 overflow-auto max-h-40">
